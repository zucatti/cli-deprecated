...
Auth = {
	user: function (profile, fn) {
		...
		if (profile.provider == "github") var typ = "github";
		if (profile.provider == "gitlab") var typ = "gitlab";
	...
	, login: function (profile, auth_type, cb) {
		...
		if (fs.existsSync(PROJECT_WEB + path.sep + "Contents" + path.sep + "Auth" + path.sep + off + ".js")) {
			...
			Auth.using = function (unit) {
				if (fs.existsSync(__dirname + path.sep + 'node_modules' + path.sep + unit)) return require(__dirname + path.sep + 'node_modules' + path.sep + unit);
				else return require(PROJECT_HOME + path.sep + 'bin' + path.sep + 'node_modules' + path.sep + unit);
			};
...
function getView(v, t) {
	...
	t.push(workspace + "view" + path.sep + v.trim() + ".js");
	//console.log(t);
};
...
function getController(controller) {
	...
	for (var i = 0; i < views.length; i++) {
		...
		PROCESSING_VIEW.push(v.trim());
	};
	nview = getDistinctArray(nview);
	for (var i = 0; i < nview.length; i++) {
		result += fs.readFileSync(nview[i].replace(/\\/g, require('path').sep), "utf-8") + "\n";
	};
	if (fs.existsSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "objects.js")) fs.appendFileSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "objects.js", result);
	else fs.writeFileSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "objects.js", result);
};
...
function make_app() {
	...
	if (fs.existsSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "bootstrap.cache")) var bootstrap = fs.readFileSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "bootstrap.cache", "utf-8");
	else {
...
function make_culture() {
	...
	for (var i = 0; i < Settings.LANGS.length; i++) {
		...
		if (fs.existsSync(PROJECT_WEB + path.sep + "Contents" + path.sep + "Culture" + path.sep + Settings.LANGS[i] + ".js")) _LANG[_LANG.length] = PROJECT_WEB + path.sep + "Contents" + path.sep + "Culture" + path.sep + Settings.LANGS[i] + ".js";
	};
...
function getRemoteCSS(url, callback) {
	...
	async.map(u, getURL, function (err, result) {
		...
		async.map(o, loadBase64Image, function (err, r) {
			...
			if (fs.existsSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "Resources")) fs.appendFileSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "Resources", result);
			else fs.writeFileSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "Resources", result);
...
function getLocalCSS(url, callback) {
	...
	if (fs.existsSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "Resources")) fs.appendFileSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "Resources", css);
	else fs.writeFileSync(PROJECT_DEV + path.sep + "webapp" + path.sep + "Resources", css);
	callback(null, css);
};
...
function update_npm() {
	if (fs.existsSync(PROJECT_HOME + path.sep + 'bin' + path.sep + 'package.json')) {
		...
		for (var i = 0; i < list.length; i++) {
			console.log('npm install ' + list[i]);
			if (process.args.sandbox) shelljs.exec(__dirname + path.sep + 'nodejs' + path.sep + 'bin' + path.sep + 'npm install ' + list[i].trim(), {
				silent: true
			});
			else shelljs.exec('npm install ' + list[i].trim(), {
				silent: false
			});
		};
...
function build_production() {
	...
	Request('https://github.com/Omneedia/tpl.omneedia.production/archive/master.zip').on('end', function () {
		...
		readStream.pipe(unzip.Parse()).pipe(writeStream.on('close', function () {
			...
			if (fs.existsSync(PROJECT_HOME + path.sep + 'src' + path.sep + 'Contents' + path.sep + 'Auth' + path.sep + 'Profiler.json')) shelljs.cp(PROJECT_HOME + path.sep + 'src' + path.sep + 'Contents' + path.sep + 'Auth' + path.sep + 'Profiler.json', _cdir + path.sep + 'auth' + path.sep + 'Profiler.json');
			...
			for (var i = 0; i < files.length; i++) Files.push({
				name: files[i].split('production' + path.sep)[1]
				, path: files[i]
			});
			Files.push({
				name: Manifest.icon.file
				, path: PROJECT_HOME + path.sep + Manifest.icon.file
			});
...
function do_get() {
	...
	if (process.args.sandbox) {
		if (op.indexOf('://') > -1) var ppmine = op.substr(op.lastIndexOf('/') + 1, op.length);
		else ppmine = op;
...
function DB_UPLOAD_SANDBOX(host, up, bar, __PID, PACKAGE_NAME, ndx, cb) {
	var uri = up[ndx];
	...
	if (uri.indexOf('?') > -1) {
		var zuri = uri.substr(0, uri.indexOf('?')).split('/');
		if (zuri.length == 3) {
			obj.file = fs.createReadStream(uri.split('?')[1]);
			obj.uri = '/' + zuri[1] + '/' + zuri[2];
			obj.db = zuri[3];
		}
		else {
			obj.file = fs.createReadStream(uri.split('?')[1]);
		...
		obj.uri = uri.substr(0, uri.lastIndexOf('/'));
		obj.db = uri.substr(uri.lastIndexOf('/') + 1, uri.length);
	};
	console.log(obj);
	//console.log(sandbox.uri+obj.uri);
	Request({
		url: sandbox.uri + obj.uri
		, formData: obj
		, method: "post"
		, encoding: null
		...
		console.log(obj);
		bar.op();
		DB_UPLOAD_SANDBOX(host, up, bar, __PID, PACKAGE_NAME, ndx + 1, cb);
	});
};
...
function do_push() {
	...
	if (__PID) {
		if (__PID == "") console.log("  You are not logged in. Sorry!".yellow);
		else {
			console.log("  Deploying package " + PACKAGE_NAME);
			Request({
				url: sandbox.uri + "/sandbox/free"
				, method: "get"
				, encoding: null
			}, function (err, resp, body) {
				...
				if (response) {
					if (!fs.existsSync(__dirname + path.sep + '.tmp')) fs.mkdirSync(__dirname + path.sep + '.tmp');
						...
						shelljs.rm('-Rf', __dirname + path.sep + '.tmp');
						fs.mkdirSync(__dirname + path.sep + '.tmp');
					};
					shelljs.cp('app.manifest', __dirname + path.sep + '.tmp' + path.sep + "app.manifest");
					shelljs.cp('app.png', __dirname + path.sep + '.tmp' + path.sep + "app.png");
					shelljs.cp('README.md', __dirname + path.sep + '.tmp' + path.sep + "README.md");
					shelljs.cp('.gitignore', __dirname + path.sep + '.tmp' + path.sep + ".gitignore");
					shelljs.cp('.style', __dirname + path.sep + '.tmp' + path.sep + ".style");
					shelljs.cp('.template', __dirname + path.sep + '.tmp' + path.sep + ".template");
					shelljs.cp('-R', 'src', __dirname + path.sep + '.tmp');
					if (!fs.existsSync(__dirname + path.sep + '.db')) fs.mkdirSync(__dirname + path.sep + '.db');
					else {
						shelljs.rm('-Rf', __dirname + path.sep + '.db');
						fs.mkdirSync(__dirname + path.sep + '.db');
					}
					...
					var Files = [];
					for (var i = 0; i < files.length; i++) Files.push({
						name: files[i].split('.tmp' + path.sep)[1]
						, path: files[i]
					});
					azip.addFiles(Files, function (err) {
						if (err) {
							shelljs.rm('-Rf', '.tmp');
							return console.log("ERR: unable to deploy", err);
						};
						var buff = azip.toBuffer();
						fs.writeFile('snapshot.' + unik, buff, function () {
							shelljs.rm('-Rf', '.tmp');
							if (!ocfg.current["deploy.host"]) {
								var sandbox = {
									uri: "http://sandbox.omneedia.com"
							...
							Request({
								...
								if (err) {
									...
									if (Manifest.platform == "mobile") var mobi = "/connect";
									else var mobi = "";
									var texto = '  Done. Check your project status at: http://deploy.omneedia.com';
									var texto2 = "\n\n";
					...
					console.log("  !!! No sandbox engine available. Please try again later.\n".yellow)
				}
			});
		}
	}
				...
				fs.writeFile(unik + '.snapshot', buff, function () {
					...
					Request({
						...
						if (!err) {
							...
							if (!r.result) {
								if (r.msg == "ALREADY_UPLOADED") console.log('  ! ERR: This version has already been published.\n'.yellow);
								if (r.msg == "PROCESS_ALREADY_RUNNING") console.log('  ! ERR: This package is already in a build process. Cancel the building task in your deployment console before continue.\n'.yellow);
							}
							else {
								console.log('  Your project has been queued for building and you can manage it on your deployment-console.\n');
							}
						}
...
function do_push_production() {
	...
	if (__PID) {
		if (__PID == "") console.log("  You are not logged in. Sorry!".yellow);
			...
			var uniqueid = require('node-uuid');
			var unik = uniqueid.v4();
			shelljs.exec('7z a snapshot.' + unik + ' .tmp', {
				silent: true
			});
			shelljs.rm('-Rf', '.tmp');
			if (!ocfg.current['publish.host']) {
...
function App_Migration_Db() {
	function dumpdata(mydb, c, tb, cb) {
		var mysql = require('mysql2');
		var connection = mysql.createConnection(MSettings.db[c].uri);
		connection.connect();
		console.log('  - Dumping ' + tb);
		connection.query('SELECT * FROM ' + tb, function (err, rows, fields) {
			if (err) throw err;
			if (!fs.existsSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.data')) fs.mkdirSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.data');
			fs.writeFileSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.data' + path.sep + tb + ".json", JSON.stringify(rows, null, 4));
			console.log('  Done.');
			cb();
		});
		connection.end();
	...
	function dumpSchemes(c, mydb, tables, i, cb) {
		var SCHEME = [];
		...
		if (tb.length == 3) {
			...
			console.log('  Done.');
			dumpdata(mydb, c, tb, function () {
				if (i + 1 < tables.length) dumpSchemes(c, mydb, tables, i + 1, cb);
				else cb();
			});
		};
	};

	function dump(k, dbo, cb) {
		var mydb = dbo[k];
		console.log('  - Creating Class ' + mydb);
		var c = -1;
		for (var j = 0; j < MSettings.db.length; j++) {
			if (MSettings.db[j].name == mydb) c = j;
		};
		var setup = MSettings.db[c].uri;
		var cmd = ["mysqldump --single-transaction -d " + setup.split('@')[1].split('/')[1]];
		setup = setup.split('://');
		setup = setup[1];
		...
		var port = "3306";
		try {
			if (hosts.split(':')[1]) port = hosts.split(':')[1];
		}
		catch (e) {
			port = "3306";
		};
		cmd.push('-h ' + host + ' -P ' + port);
		var users = setup.split('@')[0];
		var user = '-u ' + users.split(':')[0];
		if (users.split(':')[1]) var password = '-p' + users.split(':')[1];
		else var password = "";
		cmd.push(user);
		cmd.push(password);
		//console.log(cmd.join(' '));
		var sql = shelljs.exec(cmd.join(' '), {
			silent: true
		}).output;
		var tables = sql.split('CREATE TABLE ');
		if (fs.existsSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.scheme')) fs.unlinkSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.scheme');
		if (fs.existsSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.data')) fs.unlinkSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.data');
		dumpSchemes(c, mydb, tables, 1, function () {
			console.log('  Done.');
			var AdmZip = require('adm-zip');
			/*var zipme = new AdmZip();
			zipme.addLocalFile(PROJECT_HOME+path.sep+"src"+path.sep+'Contents'+path.sep+'Db'+path.sep+mydb+'.data');
			zipme.writeZip(PROJECT_HOME+path.sep+"src"+path.sep+'Contents'+path.sep+'Db'+path.sep+mydb+'.data');	*/
			if (k + 1 < dbo.length) dump(k + 1, dbo, cb);
			else cb();
		});
	};
...
function App_Model_Db() {
	...
	for (var i = 0; i < dbo.length; i++) {
		...
		if (cxx.split('@')[1].split('/')[0].split(':').length > 1) var port = cxx.split('@')[1].split('/')[0].split(':')[1];
		else var port = 3306;
		...
		if (fs.existsSync(PROJECT_HOME + path.sep + "src" + path.sep + 'Contents' + path.sep + 'Db' + path.sep + mydb + '.scheme')) {
			...
			for (var ii = 0; ii < texto.length - 1; ii++) {
				...
				for (var k = 0; k < LINKS.length; k++) {
					if (_IMPORT[LINKS[k].tb]) _IMPORT[maclasse].belongsTo(_IMPORT[LINKS[k].tb], {
						as: LINKS[k].as
					});
					else console.log(LINKS[k].as);
				};
...
function AppUpdate(zzz) {
	...
	App_Update(zzz, function () {
		if (fs.existsSync(PROJECT_HOME + require('path').sep + 'etc' + require('path').sep + 'db.json')) var x = JSON.parse(fs.readFileSync(PROJECT_HOME + require('path').sep + 'etc' + require('path').sep + 'db.json'));
		else var x = {};
		if (fs.existsSync(PROJECT_HOME + require('path').sep + 'db' + require('path').sep + 'db.json')) var y = JSON.parse(fs.readFileSync(PROJECT_HOME + require('path').sep + 'db' + require('path').sep + 'db.json'));
		else var y = {};
		var is_new = -1;
		shelljs.exec('git config user.name "' + Manifest.author.name + '"');
		shelljs.exec('git config user.email "' + Manifest.author.mail + '"');
...
figlet(' omneedia', {
	...
	if (process.args.mysql) {
		...
		if (def == "start") {
			if (!isWin) {
				...
				shelljs.exec('nohup "' + __dirname + path.sep + 'mysql' + path.sep + 'bin' + path.sep + 'mysqld" --defaults-file="' + userdirdata + path.sep + 'my.ini" -b "' + __dirname + path.sep + 'mysql" --datadir="' + data + '" &>"' + userdirdata + path.sep + "my.log" + '" & echo $! > "' + pid + '"', {
					silent: true
				});
				...
				spawn(userdirdata + path.sep + 'mysql.cmd', [], {
					detached: false
				});
		...
		if (def == "stop") {
			...
			if (fs.existsSync(pid)) {
				...
				if (!isWin) {
					...
					shelljs.exec('taskkill /F /PID ' + _pid, {
						silent: true
					});
				};
	...
	if (process.args.code) {
		...
		if (os.platform().indexOf('darwin') > -1) exec(__dirname + path.sep + 'brackets' + path.sep + 'Brackets.app' + path.sep + 'Contents' + path.sep + 'Resources' + path.sep + 'brackets.sh', [PROJECT_HOME]).unref();
			...
			spawn(__dirname + path.sep + 'brackets.cmd', [PROJECT_HOME], {
				detached: false
			});
		}
	};
	...
	function process_api(d, i, batch, res) {
		...
		if (i >= d.length) {
			...
			if (api.action == "__QUERY__") var x = require(__dirname + path.sep + "node_modules" + path.sep + "db" + path.sep + "__QUERY__.js");
			else {
				...
				x.getFile = function (filename, cb) {
					if (fs.existsSync(filename)) cb(filename);
					else cb(false);
				};
				x.using = function (unit) {
					if (fs.existsSync(__dirname + path.sep + 'node_modules' + path.sep + unit)) return require(__dirname + path.sep + 'node_modules' + path.sep + unit);
					else {
						if (fs.existsSync(PROJECT_HOME + path.sep + 'bin' + path.sep + 'node_modules' + path.sep + unit)) return require(PROJECT_HOME + path.sep + 'bin' + path.sep + 'node_modules' + path.sep + unit);
						else {
	...
	if (argv.indexOf('start') > -1) {
		...
		*/
		...
		if (fs.existsSync(__dirname + path.sep + ".." + path.sep + "omneedia.github.io")) app.use('/cdn', express.static(__dirname + path.sep + ".." + path.sep + "omneedia.github.io"));
		...
		var session = Session({
			store: new SessionStore({
				path: __dirname + path.sep + 'tmp' + path.sep + 'sessions'
			})
			, secret: 'pass'
			, resave: true
		...
		if (MSettings.auth) {
			...
			app.get('/account', ensureAuthenticated, function (req, res) {
				...
				if (fs.existsSync(PROJECT_WEB + path.sep + "Contents" + path.sep + "Auth" + path.sep + 'Profiler.json')) {
					...
					for (var el in profiler.profile) {
						var p = profiler.profile[el];
						if (p.indexOf(req.user) > -1) response.push(el);
					};
				};
			...
			if (MSettings.auth.github) {
				var google = MSettings.auth.github;
				console.log(MSettings.auth.github.scope);
				authom.createServer({
					service: "github"
					, id: MSettings.auth.github.key
					, secret: MSettings.auth.github.secret
					, scope: MSettings.auth.github.scope
				})
			};
			if (MSettings.auth.gitlab) {
				var google = MSettings.auth.gitlab;
				authom.createServer({
					service: "gitlab"
					, id: MSettings.auth.gitlab.key
					, secret: MSettings.auth.gitlab.secret
					, protocol: MSettings.auth.gitlab.protocol
					, host: MSettings.auth.gitlab.host
				})
			};
		...
		app.get('/tmp/:uid', function (req, res) {
			...
			if (!fs.existsSync(file)) {
				...
				if (ext == "text/html") res.end(require('fs').readFileSync(file, 'utf-8'))
					...
					if (url.length >= 1) {
						try {
							...
							for (var i = 0; i < url.length; i++) {
								...
								if (response.params.indexOf(elem[0]) == -1) res.end('param ' + elem[0] + ' not found');
								else {
									if (elem[1].substr(0, 1) == "[") response.param[elem[0]] = eval(decodeURI(elem[1]));
									else
									if (elem[1].substr(0, 1) == "{") response.param[elem[0]] = JSON.parse(decodeURI(elem[1]));
									else response.param[elem[0]] = elem[1];
								}
							};
						}
						catch (e) {
		...
		app.post('/auth/q', function (req, res) {
			...
			var $user = "x";
			if (fs.existsSync(PROJECT_VAR + path.sep + "udid" + path.sep + $_INFO + path.sep + "default")) $user = fs.readFileSync(PROJECT_VAR + path.sep + "udid" + path.sep + $_INFO + path.sep + "default", "utf-8");
		...
		app.get('/auth/login', function (req, res) {
			...
			Auth.login(req, Manifest.officer, $_LOGIN, $_PASSWORD, function (checked) {
				...
				if (!$_RESPONSE.authenticate) res.send(401);
					...
					res.header("Content-Type", "application/json; charset=utf-8");
					res.send(JSON.stringify($_RESPONSE, null, 4));
				}
			});
		});
		...
		App_Update('', function () {
			if (fs.existsSync(PROJECT_SYSTEM + path.sep + "app.js")) {
				...
				_App.upload = {
					...
					, toBase64: function (filename, cb) {
						...
						if (fs.existsSync(path)) {
							...
							console.log(new Buffer(bin, 'binary'));
				...
				for (var i = 0; i < Settings.API.length; i++) {
					if (Settings.API[i] == "__QUERY__") _App[Settings.API[i]] = require(__dirname + path.sep + 'node_modules' + path.sep + 'db' + path.sep + '__QUERY__.js');
					else _App[Settings.API[i]] = require(PROJECT_SYSTEM + path.sep + '..' + path.sep + 'Contents' + path.sep + 'Services' + path.sep + Settings.API[i] + '.js');
					var self = _App[Settings.API[i]].model = {
						...
						, fields: {
							add: function (o) {
								if (o === Object(o)) self._model.metaData.fields.push(o);
								else {
									var t = o.split(',');
									if (t.length == 3) {
										var o = {
			...
			if (process.args.sandbox) {
				...
				if (!jsoconf.cluster_host) var cluster_host = jsoconf.cluster.split(':')[0];
				else var cluster_host = jsoconf.cluster_host.split(':')[0];
				var cluster_port = "80";
				...
				socket.on('connect', function () {
					...
					socket.emit('ONLINE', {
						...
						, uri: process.args.uri
					});
					socket.on('REGISTER', function () {
						console.log(' Registered at http://' + process.uri + '');
